...
class MenuModel extends CommonModel {
    ...
    public function checkParentid($parentid) {
        $find = $this->where(array("id" => $parentid))->field("parentid");
        if ($find) {
            $find2 = $this->where(array("id" => $find))->field("parentid");
            if ($find2) {
                $find3 = $this->where(array("id" => $find2))->field("parentid");
                if ($find3) {
                    return false;
                }
            }
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $a) {
                ...
                $array = array(
                    ...
                    "name" => $a['name'],
                    "parent" => $parent,
                    "url" => url("{$name}/{$model}/{$action}{$fu}", array("menuid" => $id),false),
    ...
    public function formatMenuTree(){
        ...
        foreach ($result as $n=> $r) {
            $temp[$n] = $r->toArray();
            $temp[$n]['level'] = $this->_get_level($r['id'], $newmenus);
            $temp[$n]['parentid_node'] = ($r['parentid']) ? ' class="child-of-node-' . $r['parentid'] . '"' : '';
            $temp[$n]['str_manage'] = '<a href="' . url("Menu/add", array("parentid" => $r['id'], "menuid" => input("get.menuid"))) . '">'.lang('ADD_SUB_MENU').'</a> | <a href="' . url("Menu/edit", array("id" => $r['id'], "menuid" => input("get.menuid"))) . '">'.lang('EDIT').'</a> | <a class="js-ajax-delete" href="' . url("Menu/delete", array("id" => $r['id'], "menuid" => input("get.menuid")) ). '">'.lang('DELETE').'</a> ';
            $temp[$n]['status'] = $r['status'] ? lang('DISPLAY') : lang('HIDDEN');
            if(config('app_debug')){
                $temp[$n]['app']=$r['app']."/".$r['model']."/".$r['action'];
            }
        }
